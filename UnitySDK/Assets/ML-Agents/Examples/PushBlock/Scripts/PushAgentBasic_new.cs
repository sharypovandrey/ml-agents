...
public class PushAgentBasic : Agent
{
    ...
    public Vector3 GetRandomSpawnPosForGoal()
    {
        var foundNewSpawnLocation = false;
        var randomSpawnPos = Vector3.zero;
        while (foundNewSpawnLocation == false)
        {
            ...
            var randomPosZ = Random.Range(-areaBounds.extents.z * m_Academy.spawnAreaMarginMultiplier,
                areaBounds.extents.z * m_Academy.spawnAreaMarginMultiplier);
            randomSpawnPos = ground.transform.position + new Vector3(randomPosX, 0.0f, randomPosZ);
            if (Physics.CheckBox(randomSpawnPos, new Vector3(2.5f, 0.01f, 2.5f)) == false)
            {
                foundNewSpawnLocation = true;
            }
    ...
    {
        // Get a random position for the block.
        // goal.transform.position = GetRandomSpawnPosForGoal();
    ...
    {
        int maxPoints = 4000;
        m_RayAngles = new float[maxPoints];
        for (int i = 0; i < maxPoints; i++)
        {
            m_RayAngles[i] = i * (180f / maxPoints);
        };
    }
